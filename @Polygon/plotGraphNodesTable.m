function fig = plotGraphNodesTable(PG,cont,finger,nodes,otherFingerRelative,ContactHeight)
...
Distance = findInnerPoints3(PG,otherFingerRelative,ContactHeight,thetav);
ax(2) = axes;
hold on
...
thetav = -pi():0.05:pi();
Distance = findInnerPointsTable(PG,ContactHeight,thetav);
ax(2) = axes;
hold on
tableCont = contour(PG.S,thetav,-100*Distance.',[0,0],'r');
colormap(ax(2),[0 0 0])
%move x- and y-axis of axes2
pos=get(ax(1),'Position');
set(ax,'Position',pos);
...
for i =1:numel(cont)
    stmp = cont{i}(finger,:);
    thetatmp = cont{i}(4,:);
    plot(stmp,thetatmp,'b.','lineWidth',1)
end
