...
init_param = [];
for i = 1:N
    p2 = [sigma(i)*cos(phi(i));sigma(i)*sin(phi(i))];
    R = [cos(theta) -sin(theta);sin(theta) cos(theta)];
...
%     eq1 = com+R*vj;
%     eq2 = com+R*(vk+a*tk)-p2;
    
%     tic
%     if ~isempty(init_param)
%     [cyv,cyx,thetav,av] = vpasolve([eq1;eq2]==0,[cy,cx,theta,a],init_param);
%     else
%       [cyv,cyx,thetav,av] = vpasolve([eq1;eq2]==0,[cy,cx,theta,a]);
%       if all(av<0 | av>1) && ~isempty(thetav)
%          [cyv,cyx,thetav,av] = vpasolve([eq1;eq2]==0,[cy,cx,theta,a],[0,0,thetav(1)-pi,0.5]);
%       end
%     end
%     toc
    solution = find(av<=1&av>=0);
%     check = subs((R*J*tk).'*[0;1],theta,thetav);
%     check = find(check>0);
%     solution = intersect(check,solution);
    if ~isempty(solution) % it is possible for the equations to not have relevant solutions.
%         init_param = [cyv,cyx,thetav,av];
        pp(i) = cyv(solution(1));
        if vertfinger == 2 %(need to correct for finger movement)
            pp(i) = cyv(solution(1))-p2(2); %add the second finger's height
        end
    else %remove the point from the value lists
        remove = [remove i];
    end
end
sigma(remove) = [];
