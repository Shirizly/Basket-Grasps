...
%%
for finger = 1:2
    graphfig{finger} = plotGraphNodes(PG,cont,finger,nodes);
end
%%
starting_node = [2,1];


[Open_list,Closed_list,A,index_list,type_list,path_list] = GraphSearch_new(PG,cont,nodes,starting_node,f1,f2);
%%
...
for i=1:length(Closed_list)
    ...
    if cur_node_type<4
        ...
        hold on
        plot(s_origin,theta_origin,'ok','markerSize',14,'lineWidth',3)
        s_origin(2) =node_par(2);
        set(groot,'CurrentFigure',graphfig{2})
        hold on
        plot(s_origin(2),theta_origin,'ok','markerSize',14,'lineWidth',3)
    else
        finger = node_par(end);
    ...
    for j = 1:length(connect_ind)
        ...
        if node_type<4 % next node is DS_node
            ...
            if isempty(finger) % current node is DS_node
                set(groot,'CurrentFigure',graphfig{1})
                hold on
                plot([s(1),s_origin(1)],[theta,theta_origin],'r-','lineWidth',4)
                plot(s(1),theta,'or','markerSize',14)
                
                set(groot,'CurrentFigure',graphfig{2})
                hold on
                plot([s(2),s_origin(2)],[theta,theta_origin],'r-','lineWidth',4)
                plot(s(2),theta,'or','markerSize',14)
            else % current node is SS_node
                set(groot,'CurrentFigure',graphfig{finger})
                hold on
                plot([s(finger),s_origin],[theta,theta_origin],'r-','lineWidth',4)
                plot(s(finger),theta,'or','markerSize',14)
            end
            ...
            plot([s,s_origin_temp],[theta,theta_origin],'r-','lineWidth',4)
            plot(s,theta,'or','markerSize',14)
        end
    end
end
hold off 
