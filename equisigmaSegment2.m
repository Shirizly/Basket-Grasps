function segment = equisigmaSegment2(PG,s1init,s2init,theta0,prevedge,direction)
% This function recievs as input a point on the U_12 plane, and an
% associated angle, and constructs a segment of the sigma-contour that
% reaches the nearest rectangle edge. The segment is constructed according
% to inputted direction, making sure not to go backwards to a previous
% rectangle using prevedge. The resulting segment contains information
% about its endpoints.
% tic
% segment = [];
% disp('seg')
s10 = s1init; s20 = s2init;
% if the point given is on the boundary of the cyclic axes in U_12, go to
% other side instead, as that is where the segment will begin
s10 = s10 + (s10==0)*(PG.S(end)-s10)+(s10==PG.S(end))*(-s10);
s20 = s20 + (s20==0)*(PG.S(end)-s20)+(s20==PG.S(end))*(-s20);
s1prev = s10;
s2prev = s20;
[O1,O2] = PG.get('2pos',s10,s20);
sigma = norm(O2-O1);
relvector = O2-O1;
Rt0 = [cos(theta0) -sin(theta0);sin(theta0) cos(theta0)];
relvector = Rt0*relvector;
% syms t
...
% try
    ...
    if norm(prevedge)==0 && direction == -1
        if any(PG.S(PG.VL)==s10)
            edge1 = edge1-1;
        end
        if any(PG.S(PG.VL)==s20)
            edge2 = edge2-1;
        end
    end
...
% To reduce computing time, replace (building and) evaluating symbolic function with
% simple computing of the expression.
        dsmatrix =  [(relvector(2)*(n2(2)*cos(Sv) - n2(2) + n2(1)*sin(Sv)))/(n1(1)*n2(2) - n1(2)*n2(1)) - (relvector(1)*(n2(1) - n2(1)*cos(Sv) + n2(2)*sin(Sv)))/(n1(1)*n2(2) - n1(2)*n2(1));...
 (relvector(2)*(n1(2)*cos(Sv) - n1(2) + n1(1)*sin(Sv)))/(n1(1)*n2(2) - n1(2)*n2(1)) - (relvector(1)*(n1(1) - n1(1)*cos(Sv) + n1(2)*sin(Sv)))/(n1(1)*n2(2) - n1(2)*n2(1))];
 ...
 (relvector(2)*(n1(2)*cos(Sv) - n1(2) + n1(1)*sin(Sv)))/(n1(1)*n2(2) - n1(2)*n2(1)) - (relvector(1)*(n1(1) - n1(1)*cos(Sv) + n1(2)*sin(Sv)))/(n1(1)*n2(2) - n1(2)*n2(1))];
        ...
        while s1vertex==s2vertex && abs(Sv(end)-Sv(end-1))>1E-8 %curve segments exits rectangle too close to corner to be clear which edge is crossed
            ...
            dsmatrixtag = [(relvector(2)*(n2(2)*cos(Svtag) - n2(2) + n2(1)*sin(Svtag)))/(n1(1)*n2(2) - n1(2)*n2(1)) - (relvector(1)*(n2(1) - n2(1)*cos(Svtag) + n2(2)*sin(Svtag)))/(n1(1)*n2(2) - n1(2)*n2(1));...
 (relvector(2)*(n1(2)*cos(Svtag) - n1(2) + n1(1)*sin(Svtag)))/(n1(1)*n2(2) - n1(2)*n2(1)) - (relvector(1)*(n1(1) - n1(1)*cos(Svtag) + n1(2)*sin(Svtag)))/(n1(1)*n2(2) - n1(2)*n2(1))];
...
%             set(0, 'CurrentFigure', contspace);
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        segment = {[theta0prev,theta0],[edge1,edge2],[s1prev,s2prev],[s10,s20]};
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        segment = {theta0,[edge1,edge2],[Sv(1),Sv(end)],[s1prev,s2prev],[s10,s20]};
    end
...
%     end
    ...
    s2vertexalt = [];
    hold on
     plot(s1prev+dsmatrix(1,:),s2prev+dsmatrix(2,:),'k','linewidth',2)
% catch errmsg
%     errmsg.message
%     errmsg.stack.line
%     disp(errmsg)
